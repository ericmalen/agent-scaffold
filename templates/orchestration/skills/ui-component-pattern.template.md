...
description: Procedure for adding or modifying a component in the <!-- ai-kit:slot:stack --> layer at <!-- ai-kit:slot:layer-path -->. Use when a UI component in that layer must be created or changed and should match the codebase's existing component pattern. Not for API endpoints, styling-system overhauls, or layers outside <!-- ai-kit:slot:layer-path -->.
---

How the ui-engineer specialist adds or modifies components in
`<!-- ai-kit:slot:layer-path -->` (<!-- ai-kit:slot:stack -->).

## Procedure

1. Find an existing component as the pattern. Pick one under
   `<!-- ai-kit:slot:layer-path -->` that is closest in kind to what you are
