---
name: api-testing
description: Procedure for testing API endpoint changes in the <!-- ai-kit:slot:stack --> layer at <!-- ai-kit:slot:layer-path -->. Use when an endpoint, handler, or route in that layer is added or modified and its behavior must be proven with tests. Not for UI testing, schema migrations, or layers outside <!-- ai-kit:slot:layer-path -->.
---

How the api-engineer specialist tests endpoint changes in
`<!-- ai-kit:slot:layer-path -->` (<!-- ai-kit:slot:stack -->).

## Procedure

1. Locate the existing test patterns. Find tests under
   `<!-- ai-kit:slot:layer-path -->` that exercise endpoints similar to the
...
4. Run `<!-- ai-kit:slot:test-cmd -->`. Fix failures before reporting.
