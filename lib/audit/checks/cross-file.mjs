...
export function checkCrossFile(manifest, consumerRoot, installedSkillDirs, { aiKitRoot, registry } = {}) {
  ...
  if (aiKitRoot && registry && aiKitRoot === consumerRoot) {
    const registeredSkills = new Set([
      ...registry.baseSkills(),
      ...registry.optInSkills().map(s => s.id),
    ]);
    ...
    if (exists(skillsDir)) {
      for (const absPath of walkFiles(skillsDir)) {
        ...
        const parts = absPath.replace(skillsDir + '/', '').split('/');
        const skillId = parts[0];
        if (!registeredSkills.has(skillId)) {
          findings.push(f({
            id: 'skill-not-registered',
            severity: 'info',
            file: `.claude/skills/${skillId}/SKILL.md`,
            message: `Skill "${skillId}" is not registered in ai-kit.config.json and will not be shipped.`,
