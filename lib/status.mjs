...
export function status(flags, aiKitRootOverride) {
  ...
  if (!manifest) {
    log.info('Not initialized. Run `ai-kit init` to install ai-kit.');
    return;
  }

  log.header('ai-kit Status');

  // Version comparison
  const upstreamSha = headSha(aiKitRoot);
  ...
  if (!upstreamSha) {
    ...
    log.info("Run `ai-kit update` to install latest changes.");
  }

  if (isDirty(aiKitRoot)) log.warn('ai-kit repo has uncommitted changes.');
  ...
  if (manifest.pendingIntegration.length > 0) {
    ...
    for (const p of manifest.pendingIntegration) {
      log.warn(`  ${p.managedPath}  (ai-kit version: ${p.sidecarPath})`);
    }
  ...
  if (manifest.preexistingUnmanaged.length > 0) {
    ...
    log.info('These files were present before init. Not managed by ai-kit.');
  }
}
