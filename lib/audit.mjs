...
function checkAgent(absPath, rel) {
  const text = readText(absPath);
  if (!text) return [];
  const findings = [];
...
function checkSkill(absPath, rel) {
  const text = readText(absPath);
  if (!text) return [];
  const findings = [];
  const { frontmatter } = parseFrontmatter(text);
  ...
  // Flag namespace-style prefixes: first segment ≥5 chars that reads as a
  // product/company name rather than a common English qualifier word.
  // Common qualifiers (new, git, layer, sub, pre, etc.) are intentional, not prefixes.
  if (actualName && /^[a-z]{5,}-[a-z]/.test(actualName)) {
    const firstSeg = actualName.split('-')[0];
    const commonQualifiers = new Set(['layer', 'super', 'micro', 'macro', 'cross', 'multi', 'inter', 'intra', 'extra', 'ultra', 'under', 'outer', 'inner', 'quick', 'smart', 'clean', 'light', 'heave', 'rough', 'draft']);
    if (!commonQualifiers.has(firstSeg)) {
      findings.push(finding({
        id: 'skill-name-has-namespace-prefix',
        severity: 'warning',
        file: rel,
        message: `Skill name "${actualName}" appears to use a namespace prefix ("${firstSeg}-").`,
        detail: 'Skill names should be short verbs or nouns without a product/company prefix. The .claude/skills/ directory already provides the namespace.',
        fixable: 'manual',
      }));
    }
  }
...
export async function audit(flags) {
  ...
  if (exists(agentsDir)) {
    for (const absPath of walkFiles(agentsDir)) {
      ...
      filesScanned++;
      allFindings.push(...checkAgent(absPath, rel));
    }
  }
  ...
  if (exists(skillsDir)) {
    for (const absPath of walkFiles(skillsDir)) {
      ...
      allFindings.push(...checkSkill(absPath, rel));
    }
  }
