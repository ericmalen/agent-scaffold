...
export async function update(flags, scaffoldRootOverride) {
  ...
  if (!manifest) {
    log.error('Not initialized. Run `agent-scaffold init` first.');
    process.exit(1);
  }

  const sha = headSha(scaffoldRoot);
  if (!sha) log.warn('Cannot determine scaffold commit SHA.');
  if (sha && isDirty(scaffoldRoot)) {
    log.warn(`Scaffold has uncommitted changes. Content may not match ${sha.slice(0, 7)}.`);
  }

  const shippingItems = buildShippingSet(manifest.installed, registry, scaffoldRoot);
  const currentSrcRels = new Set(shippingItems.map(i => i.srcRel));

  log.header('Updating scaffold');
  ...
  // Files no longer in the scaffold
  for (const srcRel of Object.keys(manifest.files)) {
    if (!currentSrcRels.has(srcRel)) {
      stats.removed++;
      log.warn(`  No longer in scaffold (kept): ${manifest.files[srcRel].installedAs}`);
  ...
  if (stats.removed)   log.warn(`  no longer in scaffold: ${stats.removed}`);
...
function buildShippingSet(installed, registry, scaffoldRoot) {
  const items = [];

  for (const f of registry.baseFiles()) {
    const srcAbs = join(scaffoldRoot, f);
    if (exists(srcAbs)) {
  ...
  for (const skillName of (installed.baseSkills ?? registry.baseSkills())) {
    ...
    const skillAbs = join(scaffoldRoot, skillPath);
    if (exists(skillAbs)) {
      ...
      log.warn(`Base skill "${skillName}" not found in scaffold — skipping.`);
    }
  }
  ...
  for (const agentId of (installed.agents ?? [])) {
    ...
    const agentAbs = join(scaffoldRoot, agentInfo.path);
    if (exists(agentAbs)) {
