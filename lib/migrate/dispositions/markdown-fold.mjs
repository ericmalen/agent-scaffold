...
import { rewriteMarkdownRefs } from '../rewrite-refs.mjs';
...
export function stageUnit(unit, consumerRoot) {
  ...
  for (const src of unit.sources) {
    ...
    for (const sec of srcSections) {
      ...
      // Rewrite any relative markdown links so they resolve from the target
      // file's directory instead of the source's. URLs and consumer-root
      // absolute paths are left unchanged by the helper.
      const rewritten = rewriteMarkdownRefs(block.join('\n'), src.path, unit.target).split('\n');
