...
1. Determine the type (see table in [commit-format](references/commit-format.md)).
...
5. For PRs, mirror the commit format in the title and apply the description
   template in [pr-guidelines](references/pr-guidelines.md).
6. For branches, use `<type>/<short-description>` — see
   [branch-naming](references/branch-naming.md).

## References

- [commit-format](references/commit-format.md) — full format
  spec, type vocabulary with SemVer mapping, scope, breaking-change, body and
  footer rules.
- [pr-guidelines](references/pr-guidelines.md) — PR titles,
...
- [worked-examples](examples/worked-examples.md) — good and
