...
The docs package is **baseline** ai-kit — it installs automatically when you
adopt the kit (shipped path-for-path by `install-adoption`, alongside the
`adopt-*` skills). During adoption, existing bespoke documentation rules in
your instruction files are `supersede`d by this package (your original text is
preserved in the adoption review report). To add it to an already-adopted repo
that predates baseline status, copy from the kit clone:
