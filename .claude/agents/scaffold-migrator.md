...
description: Resolves brownfield migration debt left by `agent-scaffold init`. Reads each `.scaffold` sidecar plus the consumer's original, classifies content against the scaffold's structure using deterministic rules, presents a single batch migration plan for approval, then applies every merge, deletes resolved sidecars and folded instruction files, and updates `.ai-scaffold.json` bookkeeping. Invoke to finish a brownfield migration, resolve `.scaffold` sidecar files, or clear `pendingIntegration`. Never applies an edit before the batch plan is approved.
...
`.ai-scaffold.json` bookkeeping. Never applies an edit before the batch plan is
approved.

## Procedures

This agent runs as part of a **two-phase** migration — a subagent cannot pause
for approval mid-run. The `scaffold-migrate` skill (the orchestrator) drives the
...
`SCOPE` block, and whether `.ai-scaffold-migration-plan.md` exists at the
consumer repo root:
...
written **once**, to `.ai-scaffold-staging/`; the plan file only ever carries a
...
`.claude/skills/scaffold-migrate/references/orchestration.md` — read it before
planning or applying.

### Common (all modes)

1. Locate `.ai-scaffold.json` at the consumer repo root (in scoped plan mode the
   `SCOPE` block gives `consumer-repo`; otherwise use Glob to search up). If
   absent, report "not initialized — run `agent-scaffold init` first" and stop.
...
5. Produce the **final merged content** and **write it to staging files** under
   ...
   `.ai-scaffold-staging/AGENTS.md`, `.ai-scaffold-staging/CLAUDE.md`). Create
...
10. **Write each work unit's final merged content to staging files** under
    `.ai-scaffold-staging/`, mirroring the target paths — exactly as a scoped
    worker would. Then **write `.ai-scaffold-migration-plan.md`** at the consumer
    repo root in the structure from `orchestration.md`: the move-list, premise
    snapshots, exact `.ai-scaffold.json` edits, deletions, coverage check. The
...
### Apply mode — `.ai-scaffold-migration-plan.md` EXISTS

12. Read `.ai-scaffold-migration-plan.md` (the approved plan).
...
14. **Execute the plan mechanically — no content is regenerated.** The staging
    ...
    - Apply the `## Manifest changes` to `.ai-scaffold.json` exactly as written.
    - `rm` everything in `## Deletions` — resolved sidecars and folded files,
      then the `.ai-scaffold-staging/` directory, then the plan file.
15. Print a final summary: every file moved into place, created, or deleted, and
    the resulting state of both manifest arrays. Suggest `agent-scaffold status`
    to confirm.

## Never

- Touch the real working tree in **either plan mode**. Plan modes write **only**
  under `.ai-scaffold-staging/` (and, in whole plan mode, the plan file). Staging
  files are not the working tree; real repo files, sidecars, and
  `.ai-scaffold.json` are — leave those untouched until apply.
- In **scoped plan mode**, write anything outside `staging-root`, write the plan
  file, or touch `.ai-scaffold.json`. A scoped worker produces staging files and
...
- Touch any file not named in `pendingIntegration` or `preexistingUnmanaged` —
  ...
  nested `AGENTS.md` + its sibling `CLAUDE.md`), the `.ai-scaffold-staging/`
  directory, `.ai-scaffold.json` itself, and `.ai-scaffold-migration-plan.md`.
...
- Use Bash for anything other than: in apply mode, moving staging files into
  ...
  `.ai-scaffold-staging/` directory / `.ai-scaffold-migration-plan.md`;
...
.claude/skills/scaffold-migrate/references/manifest-operations.md
.claude/skills/scaffold-migrate/references/orchestration.md
