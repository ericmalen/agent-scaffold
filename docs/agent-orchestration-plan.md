...
`ai-kit-adopt`. Verification targets are the repo fixtures (§9.5):
...
| B2   | `dependency-mapper` skill: internal package deps, key external deps per layer                                                                                                                                 | Skill md                      | Maxi-repo: maps shared→api/ui, Prisma→db correctly. Mini-repo: reports no internal deps without erroring                               |
...
| B8   | `plan-synthesizer` agent: profile + decisions → `blueprint.json`, runs handoff-validator before writing                                                                                                       | Agent md                      | Maxi-repo blueprint validates and names ≥4 specialists. Mini-repo blueprint validates and selects the generic specialist template (≥1) |
...
- Dependencies declared in manifests only — never installed; stub `build`
  scripts and Node-builtin tests keep the kit zero-dependency
