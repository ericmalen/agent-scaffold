...
Repo-agnostic agent/skill orchestration system, built in `ai-kit` and distributed
through the existing adoption installer (`scripts/install-adoption.mjs` allowlist).
Discovery runs from an open kit clone against a target path — same pattern as
`ai-kit-adopt`. First target: AI Portal monorepo (ui / api / db / shared).
...
| DD-7  | Discovery/generation outputs live in target repo at `docs/orchestration/` (`repo-profile.json`, `decisions.json` + rendered `decisions.md`, `blueprint.json`, `generation-manifest.json`).             | Docs serve human and agent audiences; lazy-loaded via `## Documents` sections.                                         |
...
per the `scripts/install-adoption.mjs` allowlist — same mechanism as the adoption
skills):**
...
- Pure functions (`scripts/lib/orchestration/`): `parseTasksMd`, `renderDecisionsMd`, strict inline instantiation in `instantiate.mjs` (DD-5; `scripts/lib/template.mjs` untouched)
...
- Fixtures: `test/fixtures/mini-repo` (tiny single-package repo for repo-agnosticism testing, §9.5)
...
## 3. Repo layout (ai-kit additions)
...
rides into targets inside `.claude/ai-kit-adoption/templates/` via the existing
wholesale copy in `install-adoption.mjs`. Pre-F1 that ride-along is harmless
dead weight: `.claude/ai-kit-adoption/` is adoption-time tooling that
`adopt-verify` removes before merge, so non-piloting adoptees never keep it.

```
ai-kit/
├── scripts/lib/
│   ├── template.mjs               # existing; untouched (adoption block routing)
│   └── orchestration/
...
│   ├── agents/                    # *.template.md with <!-- ai-kit:slot:name --> markers
│   ├── skills/
...
│                                  # decided in install-adoption.mjs, Phase F1)
└── test/
...
kit clone against targets and fixtures (the `ai-kit-adopt` pattern). Only
...
| A3   | Write `orchestration-blueprint` validator: specialists[] (name, template id, slot values, model tier, turn limit, tools[]), orchestrator config, **dispatch_rules** (DD-4 thresholds as data), docs to reference, eval requirements per agent. **No version pins — those live in the generation manifest (DD-13)**. Model tier is an enum of logical tiers (`haiku \| sonnet \| opus`), never concrete model ids — the scaffolder (C4) owns the tier→model map     | `validateBlueprint` + tests                                    | Validates a hand-written blueprint fixture; rejects 3 malformed fixtures (full-instantiation check lives in C3/C4)     |
...
| A6   | Write `templates/orchestration/docs/dispatch-rules.md` (DD-4, expanded with examples from AI Portal layers)                                                                                                                                                                                                             | Doc                                                            | Reviewed; referenced by orchestrator template                                                                          |
...
Per DD-12, every acceptance below runs against **two targets**: AI Portal and `test/fixtures/mini-repo`.

| Step | Work                                                                                                                                                                                                          | Deliverable                   | Acceptance                                                                                                                             |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| B0   | Create `test/fixtures/mini-repo`: tiny single-package Node CLI with one test, one lint config, deliberately different conventions from AI Portal (§9.5)                                                       | Fixture repo                  | Builds, tests pass, committed under `test/fixtures/`                                                                                   |
...
**Phase C done when:** AI Portal's blueprint instantiates a complete agent set, generation-manifest-tracked, reproducible, updateable.
...
| Step | Work                                                                                                                                                                                                                                                                                                                                                       | Deliverable         | Acceptance                                                  |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------------------------------- |
| F1   | Extend the `scripts/install-adoption.mjs` allowlist with orchestration assets: meta-agents/skills shipped per-asset (decided in the allowlist, the kit's existing what-ships policy); `templates/orchestration/` and `scripts/lib/orchestration/` already ride along with the wholesale `templates/` + `scripts/lib/` copies. Add a conditional **orchestration layer** section to `spec/target-layout.md` (generated agents/skills in `.claude/`, `docs/orchestration/` incl. checklists + evals, `tasks.md`) — spec changes land when the layer ships, not before | Allowlist entries + tests + spec section | Adoption of a clean repo installs the orchestration meta-kit; `spec/target-layout.md` covers the generated surfaces |
...
A1 + A3 (two validators) → minimal B4 analyst → hand-write a blueprint → C3 instantiator → generate one specialist → run it on one tiny AI Portal task.
...
- Templates: `<name>.template.md`; slots `<!-- ai-kit:slot:name -->`, kebab-case (DD-5)
...
- Meta-skill trigger descriptions must be **orchestration-scoped** — generic names (`retro`, `log-report`, `drift-checker`) collide with target-repo skills post-F1; the description, not the name, is what prevents mis-triggering
...
- Lives in `test/fixtures/mini-repo`; never ships (`test/` is never installed into targets)
